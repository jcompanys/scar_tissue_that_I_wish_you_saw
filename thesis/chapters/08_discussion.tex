...
and provides a quantitative reference for interpreting population-level scar patterns.
The concentration of burden in the inferior-to-apical
mid-ventricular wall, straddling the RCA--LCX boundary, is consistent with
the known role of the inferoseptal and inferolateral walls in re-entrant VT. The consistent sparing of the basal ring reflects the lower probability of occlusion at the
origin segments of all three coronary territories: basal myocardial involvement requires
either a very proximal occlusion (near the vessel origin, which would compromise a large
territory and is associated with higher acute mortality, making it underrepresented in
survivors) or that the ischaemic wavefront propagates far enough downstream from a distal
occlusion to reach basal tissue, both scenarios less frequent than mid-artery occlusion.
...
by developmental and functional pressures \cite{rodero2021}. Post-infarction scar
...
across individuals, invalidating fixed-axis assumptions. A fixed-axis approach,
...
in Sections~\ref{sec:scarmorph} and~\ref{sec:terrburden}: 38\% of patients (40/105)
had single-territory scar, 39\% (41/105) two-territory, and 12\% (13/105) three-territory
involvement. Single-territory patients showed the lowest burden error ($|\Delta\text{CZ}|
= 0.012$), consistent with better case-base coverage of focal patterns. Three-territory
patients showed the highest burden error (0.034), reflecting that diffuse multi-territory scar
is rare enough to lack close matches among the remaining 104 cases. Notably, segment
Dice did not decrease monotonically with territory count: three-territory patients achieved
the highest segment Dice (0.617), likely because extensive scar increases segment overlap
by chance across more involved segments. Retrieval difficulty is therefore better
characterised by burden error and by territory type than by territory count alone. This is not
a failure of the retrieval method but a faithful reflection of the population: example-based
generation is inherently limited by the diversity of the library, and its performance ceiling
is set by cohort size.
...
ones. One possible anatomical explanation is that the inferior wall may be thinner than the
anterior wall, so a necrotic wavefront of the same absolute depth could represent a higher
fractional transmurality---making inferolateral infarcts geometrically more transmural even
without greater absolute ischaemic depth. This interpretation cannot be confirmed without outcome
...
Several limitations of this work must be acknowledged. The cohort of 105 patients, while sufficient for the population-level characterisation
