\section{Polar Map Construction}
\label{sec:polarmaps}
...
representation that preserves anatomical meaning across patients. This section describes
the pipeline that constructs this projection.

\subsection{Rationale: Why Flatten to 2D}
...
LV meshes to a common template and mapping scar labels through the deformation
field \cite{doste2026}.
...
two-dimensional image suited to standard statistical analysis.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\textwidth]{figures/polar_map_steps}
    \caption{Polar map projection pipeline converting a 3D left ventricular (LV) mesh to a
    standardised 2D bull's-eye representation aligned with the American Heart Association
    (AHA) 17-segment model. From left to right: (1) 3D view with right ventricle (RV) on the
    left and LV on the right in anatomical orientation (apex inverted); (2) apex-to-base view
    after long-axis detection and apex-down re-orientation (red dot: detected apex);
    (3) circumferential angle $\theta$ coordinate coloured on the LV surface;
    (4) longitudinal coordinate $s$ ($s=0$ at apex, $s=1$ at base) coloured on the LV surface;
    (5) final bull's-eye map with AHA 17-segment overlay.}
    \label{fig:polarmaps2}
\end{figure}

\subsection{The Bull's-Eye Polar Map and the AHA 17-Segment Model}
\label{sec:aha17model}

The bull's-eye (or polar map) is a standard two-dimensional representation of the left
ventricular wall in which the apex appears at the centre of a disk and the base at the
periphery. The radial coordinate encodes apex-to-base position ($s = 0$ at apex, $s = 1$
at base) and the angular coordinate encodes circumferential location around the
ventricular wall ($\theta$).

The American Heart Association (AHA) 17-segment model \cite{cerqueira2002} divides the
LV into anatomically standardised regions defined by four concentric rings:
\begin{itemize}
    \item \textbf{Apex cap} (segment 17): the apical tip, innermost disk region.
    \item \textbf{Apical ring} (segments 13--16): four $90^{\circ}$ sectors at the apical level.
    \item \textbf{Mid-cavity ring} (segments 7--12): six $60^{\circ}$ sectors at the
          mid-ventricular level.
    \item \textbf{Basal ring} (segments 1--6): six $60^{\circ}$ sectors at the basal level.
\end{itemize}

Segments are labelled by their anatomical location: anterior, anteroseptal, inferoseptal,
inferior, inferolateral, and anterolateral (going clockwise from the septal reference).
This standardised division enables scar location, extent, and transmurality to be compared
across patients in an anatomically meaningful common frame, and forms the basis for all
territory assignments (LAD: segments 1, 2, 7, 8, 13, 14, 17; RCA: 3, 4, 9, 10, 15;
LCX: 5, 6, 11, 12, 16).

\subsection{Apex Detection via Multi-Candidate Convex-Hull Search}
...
(having similar spread in all directions, with no dominant elongation axis), causing PC1 to
align with circumferential rather than longitudinal variance. RV PC1 is
...
\subsection{Long-Axis and Septal Reference}
...
apically-extending RV free wall (the right ventricular wall extends downward toward the apex
alongside the LV, which would pull the RV centroid away from its true basal position if the
full mesh were used). The raw direction vector from the LV centroid to the RV
...
\subsection{Scar Projection Methods}
...
area per bin, removing bin-size and LV-size bias. Both area-based methods were tested
specifically to verify that the apical region --- which is compressed toward the centre of the
bull's-eye disk --- does not introduce a systematic underestimation artefact analogous to
the Antarctic distortion in cylindrical cartographic projections, where polar regions appear
disproportionately large. Consistency between all three approaches was verified to ensure
that findings are not artefacts of the binary representation (the vertex-based method assigns
each bin a value of 0 or 1 (no scar / scar present), which could in principle overstate scar
extent in sparsely-covered bins or miss graded scar burden captured by area-based methods).

\subsection{Polar Map Verification}
...
endocardial surface points within a search radius (tolerance) derived from the mean edge
length of the Core Surface mesh, so that endocardial points within that distance of any scar
vertex are considered scar-positive, then maps those labelled points using the 2D flat
coordinates. A further distinction concerns
the radial mapping: the reference method applies a radial displacement transform of the form
$r_\text{new} = r^{1/3}$ to the conformally flattened disk, which redistributes points
outward and produces denser coverage near the basal ring. The
...
% Figures moved to Results §6.2.5 (Figure~\ref{fig:verif3d} and Figure~\ref{fig:verifpolar}).
