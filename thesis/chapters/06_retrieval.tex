...
Four distance metrics were evaluated on the selected descriptor: weighted Euclidean
(equivalent to standard Euclidean after $z$-score normalisation), standard Euclidean,
...
\begin{equation}
    ...
    \label{eq:softmax}
\end{equation}
where $\tau = 1.0$ balances similarity and diversity. Every output remains a real patient scar,
but repeated calls with the same query can return different plausible cases.

\section{Conditioning on Clinical Variables}

As an exploratory extension, the retrieval system was tested using manually defined target
profiles representing simplified scar specifications such as ``small inferior scar'', ``large
anteroseptal scar'', ``moderate lateral scar'', and ``diffuse multiterritory scar''. Each profile
was encoded as a query vector with non-zero burden assigned to the relevant AHA segments.
Full conditioning on demographic variables was not implemented due to cohort size
limitations, but the retrieval framework naturally supports such conditioning by restricting the
searchable case base or by appending clinical variables to the descriptor vector.
