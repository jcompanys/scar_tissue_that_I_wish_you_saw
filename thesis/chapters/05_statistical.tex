\section{Statistical Framework for Scar Characterisation}
\label{sec:statistical}

This section describes the statistical methods used to characterise scar geometry at the
population level. The analyses proceed in three stages: (i) segment- and territory-level scar
burden quantification using the AHA 17-segment model; (ii) dimensionality reduction and
spatial correlation analysis; and (iii) transmurality and fragment-level geometric
characterisation. All methods assume non-parametric distributions, as motivated by the
normality diagnostics reported in Section~\ref{sec:normality}.

\subsection{AHA-17 Segmentation, Territory Assignment, and Normality Diagnostics}
\label{sec:normality}

Scar burden per segment was defined as the fraction of polar-map bins classified as
scar-positive within that segment. Territory burden was defined analogously over LAD
(segments 1, 2, 7, 8, 13, 14, 17; 4\,448 bins, 54.3\%), RCA (segments 3, 4, 9, 10, 15;
1\,888 bins, 23.0\%), and LCX (segments 5, 6, 11, 12, 16; 1\,856 bins, 22.7\%). A territory
was considered involved when at least 2\% of its polar bins were scar-positive. This
threshold was selected via sensitivity analysis across candidate values (1\%, 2\%, 3\%, 5\%,
10\%); the choice and sensitivity curve are discussed in the Results
(Section~\ref{sec:terrburden}).
...
\begin{table}[htbp]
    \centering
    \caption{AHA 17-segment scar burden summary ($n=105$). Segments ordered by ring (basal, mid, apical, apex).}
...
\subsection{Within-Patient Spatial Tests and Bootstrap Confidence Intervals}

Whether scar burden varies systematically across AHA segments within the same patient
was tested using the Friedman test, a non-parametric analogue of one-way repeated-measures
ANOVA (a test that compares distributions across multiple related groups---here, the 17 AHA
segments measured within the same patients). Each of the 105 patients contributed a vector
of 17 paired segment-burden values. Pairwise post-hoc comparisons were conducted using
Wilcoxon signed-rank tests with Bonferroni correction for the 136 pairwise comparisons.

Population-level confidence intervals for the mean scar burden in each AHA segment
(defined as the mean fraction of polar-map bins classified as scar-positive within that segment
across patients) were estimated via patient-level bootstrap resampling. In each of 2\,000
iterations, 105 patients were sampled with replacement---preserving each patient's full
17-segment vector---and the mean burden per segment was recomputed. The 2.5th and
97.5th percentiles defined the 95\% confidence interval. The primary purpose of this
procedure is not to establish statistical significance of individual segment means, but to
quantify the uncertainty around population estimates for the atlas and archetype maps.
Because the underlying distributions are zero-inflated and heavily skewed, symmetric
normal-theory intervals would systematically misrepresent the sampling distribution: they
produce symmetric error bars around means that are, in reality, bounded below by zero and
pulled by long positive tails. The bootstrap intervals are asymmetric by construction and
therefore faithfully reflect the shape of the sampling distribution, providing honest
uncertainty bounds for the population scar atlas.
...
\subsection{Dimensionality Reduction and Correlation Structure}
...
Component loadings were mapped back onto the AHA bull's-eye representation to visualise
...
whether multi-territory involvement is the norm.

\subsection{Population Variability and Archetype Maps}
...
area-based and coverage projections. Population-level archetype maps stratified by dominant
territory are provided in Appendix~\ref{app:archetypes}.

\subsection{Transmurality: Continuous Vertex-Depth Computation}
...
layers. However, the discrete layer resolution proved too limited: the resulting transmurality
...
spatial index (a k-d tree data structure enabling efficient nearest-neighbour queries in
three-dimensional space) is built for each surface. Every vertex of the core-zone scar mesh
is then located in 3D space, and its Euclidean distances to the nearest point on the
endocardial surface ($d_\text{endo}$) and the epicardial surface ($d_\text{epi}$) are
computed. The fractional wall depth of that vertex is defined as:
\begin{equation}
...
\subsection{Fragment-Level Characterisation}
\label{sec:fragments}
...
\subsection{Geometric Shape and Transmurality Gradient Descriptors}
...
shallow scar, or focal but deeply penetrating scar.
...
indicates that scar has a dense-core / thin-edge transmural structure, relevant for
arrhythmogenesis because the gradient zone---where transmurality transitions from thick to
thin---is consistent with the zone where conduction heterogeneity is maximal, noting that
the border zone surrounding the core---not analysed here---is the primary substrate for slow
conduction channels.
...
forms a single compact lesion or a dispersed, multi-component distribution.
\textbf{Endocardial--epicardial dominance} classifies each scar-positive bin by onset depth:
endocardial ($\leq 30\%$), midwall (30--60\%), or epicardial ($> 60\%$ of wall thickness),
distinguishing ischaemic from non-ischaemic onset patterns. \textbf{Apex-to-base gradient}
fits a first-order polynomial to row-level mean burden ($s \in [0,1]$, apex to base), with
Spearman correlation as a non-parametric summary of longitudinal scar organisation.
