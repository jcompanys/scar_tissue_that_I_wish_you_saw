...
Characterising the spatial distribution and structural properties of myocardial scar at the
population level remains a substantial methodological challenge. The difficulty arises from
the high inter-patient variability in scar location, shape, and tissue composition. In current
clinical practice, scar assessment relies predominantly on summary metrics such as LVEF
and global scar mass, which capture overall disease severity but discard the spatial and
geometric information that determines electrophysiological substrate and arrhythmic risk.
The analyses presented here aim to take a step beyond these aggregate measures by
systematically quantifying the spatial, statistical, and transmural properties of scar across the
cohort.

\section{AHA-17 Segmentation, Territory Assignment, and Normality Diagnostics}

Each binary polar map was partitioned into the 17-segment model recommended by the
AHA \cite{cerqueira2002}. Scar burden per segment was defined as the fraction of polar-map bins classified as
...
analysis across candidate values (1\%, 2\%, 3\%, 5\%, 10\%): at 1\% the multi-territory
...
were assessed using quantile--quantile plots and chi-squared goodness-of-fit plots. Based
on these diagnostics, all subsequent analyses relied on non-parametric methods and
...
extent: a segment can be frequently affected across patients yet carry low mean burden
(many patients have a small amount of scar there), or rarely affected but carrying high burden
when it is. This dissociation is a direct consequence of the zero-inflation documented above.
Because the majority of patients have zero scar in any given segment, the unconditional
mean is dragged toward zero even when the positive-only mean is substantial. To make this
structure visible, the segment-level summary tables report both quantities
separately---prevalence as the percentage of patients with any scar in the segment, and
conditional extent as the mean burden among those patients---rather than relying on a single
unconditional mean that would obscure the distinction.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/fig5b_population_scar_frequency}
    \caption{Population scar frequency map. Each bin of the $64\times128$ polar grid is coloured by the fraction of the 105-patient cohort with scar present at that location. The AHA 17-segment boundaries are overlaid. Hotspot concentrated in mid-inferolateral and mid-inferior segments (AHA 11 and 10).}
    \label{fig:stat3}
\end{figure}
